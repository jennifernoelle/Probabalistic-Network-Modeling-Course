...
-   Add question about small world path length distribution
-   Think about improving the flow of definitions
-   Explain rotation interp of spectral decomp
-   Maybe add small world sims before small world question
-   Take out some bad graphs instead explain scaling  
-   Do a better job characterizing the trade network wrt metrics
...
```{r}
# Add edge weights: log will work better for this
...
```{r}
mean(degree)
hist(degree, breaks = 10)
abline(v = mean(degree))

cbind(eigen(A)$vectors[,1], trade.countries.subset$code)
sum(eigen(A)$vectors[,1]) 
eigen(A)$values

eigen_centrality(g, weights = NA)$vector
sum(eigen_centrality(g, weights = NA)$vector)
