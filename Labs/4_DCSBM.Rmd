...
-   Add proc and accuracy for trade application
...
Now we simulate from the model. A word of warning: the (induced) prior
on $\tau$ is weakly informative, and so drawing from that prior has the
potential to generate extreme heterogeneity within communities. To
preserve the block structure for this example, we could simply set $tau$
to induce a moderate level of heterogeneity, rather than drawing it from
its prior.
...
*Answer* Tau controls the extent of heterogeneity; if it's overly large,
the block structure is no longer visible.
...
mcmc_intervals(mu.samples, prob = 0.9, prob_outer = 0.95)
...
mcmc.out <- nimbleMCMC(code = DCsbmCode, constants = DCsbmConsts,
                       ...
                       nchains = nchains, niter = niter,
                       summary = TRUE, WAIC = TRUE,
                       monitors = c('lambda', 'Pi', 'Mu', 'Z', 'Alpha', 'tau', 'logL'))
```

## MCMC and model Diagnostics 

First, get an overview of accuracy.
...
Next, we look at mixing using the (identifiable) log likelihood. 
```{r}
## MCMC diagnostics
sbm.samples <- mcmc.out$samples # keep chains separate for this part
mcmc_trace(sbm.samples, regex_pars = c("logL")) 

```
## Posterior clustering 

To review the posterior clustering, we follow the same procedure
demonstrated in the classical SBM exercises, utilizing the minimum VI to extract
a representative partition from the posterior clustering samples. 
...
p1 <- data.frame(V1 = evec.A[,1], V2 = evec.A[,2], Post.Z = as.factor(z.cl.post), 
                 code = features$code) %>% 
  ggplot(aes(x = V1, y = V2, color = Post.Z, label = code), alpha = 0.25) + 
...
Whereas in the classical SBM with strongly assortative prior, posterior
clustering is based largely on vertex degree, the more flexible DC-SBM
reveals different clusterings which are not clearly based on observable
attributes.
