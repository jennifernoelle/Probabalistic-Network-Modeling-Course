...
# Look at mixing
mcmc_trace(all.samples.glmm, regex_pars = c("beta1"))

# How does the same region coefficient change?
mcmc.glmm.out$summary$all.chains[1:10,]

# How much degree heterogeneity is there?
samples.beta2 <- all.samples.glmm[, grepl("beta2", colnames(all.samples.glmm))]
colnames(samples.beta2) <- features$code
mcmc_intervals(samples.beta2)
...
A.r <- matrix(NA, nrow = nC, ncol = nC)
postD <- matrix(NA, nrow = nrow(p.samples.glmm), ncol = 1)
allD <- matrix(NA, nrow = nrow(p.samples.glmm), ncol = nC)
for(r in 1:nrow(p.samples)){
  
  # Sample adjacency matrix
  p.post.r <- matrix(data = p.samples.glmm[r,], byrow = FALSE, nrow = nC, ncol = nC)
  diag(p.post.r) <- 0
  A.r[upper.tri(A.r)] <- rbinom(nC*(nC - 1)/2, 1, p.post.r[upper.tri(p.post.r)])
  A.r[lower.tri(A.r)] <- t(A.r)[lower.tri(A.r)]
  diag(A.r) <- 0
